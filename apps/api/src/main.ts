...
async function bootstrap() {
    ...
    const trustedProxies = configService.get('security.trustedProxies') || ['127.0.0.1'];
    ...
    app.use(
        helmet({
            contentSecurityPolicy: nodeEnv === 'production' ? helmetConfig?.contentSecurityPolicy : false,
            crossOriginEmbedderPolicy: helmetConfig?.crossOriginEmbedderPolicy ?? false,
            crossOriginResourcePolicy: helmetConfig?.crossOriginResourcePolicy ?? { policy: 'cross-origin' },
        })
    );
