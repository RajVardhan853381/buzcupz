...
export class OrdersService {
  ...
  async create(dto: CreateOrderDto, userId: string, restaurantId: string) {
    ...
    const itemsWithPrices = await this.calculateOrderItems(
      dto.items,
      restaurantId,
    );
    ...
    const maxPrepTime = Math.max(
      ...itemsWithPrices.map((i) => i.preparationTime || 15),
    );
    const estimatedReadyAt = new Date(
      Date.now() + (maxPrepTime + 5) * 60 * 1000,
    );
  ...
  async updateStatus(
    ...
    this.logger.log(
      `Order ${updatedOrder.orderNumber} status updated to ${dto.status}`,
    );
  ...
  async getTodayStats(restaurantId: string) {
    ...
    return {
      ...
      preparingOrders:
        (statusCounts["CONFIRMED"] || 0) + (statusCounts["PREPARING"] || 0),
    ...
    for (const item of items) {
      ...
      if (item.modifiers && item.modifiers.length > 0) {
        for (const modSelection of item.modifiers) {
          ...
          const modUnitPrice = this.calculateModifierPrice(
            foundModifier,
            unitPrice,
            1,
          );
          const modTotalPrice = this.calculateModifierPrice(
            foundModifier,
            unitPrice,
            qty,
          );
