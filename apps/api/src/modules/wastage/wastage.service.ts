...
export class WastageService {
    ...
    async getStats(restaurantId: string): Promise<WastageStats> {
        ...
        const [
            ...
            this.prisma.wastageRecord.aggregate({
                where: { restaurantId, wastedAt: { gte: lastWeekStart, lt: weekStart } },
                _sum: { totalCost: true },
            }),
            this.prisma.wastageRecord.aggregate({
                where: { restaurantId, wastedAt: { gte: lastMonthStart, lt: monthStart } },
                _sum: { totalCost: true },
            }),
        ...
        return {
            ...
            trends: {
                vsLastWeek: lastWeekCost > 0 ? ((weekCost - lastWeekCost) / lastWeekCost) * 100 : 0,
                vsLastMonth: lastMonthCost > 0 ? ((monthCost - lastMonthCost) / lastMonthCost) * 100 : 0,
            },
        };
    }
    ...
    async getReport(restaurantId: string, period: 'week' | 'month' = 'week') {
        ...
        const itemIds = topItems.map((t) => t.inventoryItemId).filter((id): id is string => !!id);
    ...
    async generateDailySummary() {
        ...
        for (const restaurant of restaurants) {
            try {
                ...
                this.logger.error(`Failed to generate summary for ${restaurant.id}: ${error.message}`);
            }
        }
    }
}
