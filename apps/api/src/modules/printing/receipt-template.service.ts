...
export class ReceiptTemplateService {
  formatCustomerReceipt(order: OrderWithDetails): string {
    ...
    content +=
      this.centerText(`${restaurant.city}, ${restaurant.state}`) + "\n";
  ...
  private formatLineItem(
    ...
    // Calc spaces
    const spaceLength =
      width - qtyStr.length - truncatedName.length - priceStr.length;
    return `${qtyStr} ${truncatedName}${" ".repeat(Math.max(1, spaceLength))}${priceStr}\n`;
  }

  private formatTwoColumns(
    left: string,
    right: string,
    bold = false,
    width = 48,
