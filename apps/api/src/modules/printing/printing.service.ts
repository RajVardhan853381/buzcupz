...
import { PrintJobStatus, PrinterStatus, PrinterType, PrintJobType } from '@prisma/client';
...
export class PrintingService {
    ...
    async printOrder(restaurantId: string, orderId: string, printerId?: string) {
        ...
        if (!targetPrinterId) throw new NotFoundException('No active printer found');
    ...
    async processPrintJob(jobId: string) {
        ...
        this.logger.log(`Processing print job ${job.id} for printer ${job.printer.name}`);
        ...
        try {
            if (job.printer.type === PrinterType.NETWORK) {
                ...
                this.logger.log(`[MOCK] Printing to NETWORK printer ${job.printer.ipAddress}`);
