...
import { CostCategoryType, CostFrequency, CostStatus, EmployeeType, PaymentFrequency } from '@prisma/client';
import { CreateCostCategoryDto, UpdateCostCategoryDto } from './dto/create-cost-category.dto';
import { CreateCostEntryDto, UpdateCostEntryDto } from './dto/create-cost-entry.dto';
...
export class CostManagementService {
    ...
    async updateCategory(restaurantId: string, id: string, dto: UpdateCostCategoryDto) {
    ...
    async createCostEntry(restaurantId: string, dto: CreateCostEntryDto, userId?: string) {
        const entry = await this.prisma.costEntry.create({
            data: {
                ...dto,
                date: new Date(dto.date),
                recurringStartDate: dto.recurringStartDate ? new Date(dto.recurringStartDate) : undefined,
    ...
    private getNextDate(date: Date, frequency: CostFrequency): Date {
        ...
        switch (frequency) {
            case CostFrequency.DAILY: newDate.setDate(newDate.getDate() + 1); break;
            case CostFrequency.WEEKLY: newDate.setDate(newDate.getDate() + 7); break;
            case CostFrequency.BI_WEEKLY: newDate.setDate(newDate.getDate() + 14); break;
            case CostFrequency.MONTHLY: newDate.setMonth(newDate.getMonth() + 1); break;
            case CostFrequency.QUARTERLY: newDate.setMonth(newDate.getMonth() + 3); break;
            case CostFrequency.YEARLY: newDate.setFullYear(newDate.getFullYear() + 1); break;
        }
        return newDate;
    }
    ...
    async findAllCostEntries(
        restaurantId: string,
        options: { startDate?: string; endDate?: string; categoryId?: string; status?: CostStatus; page?: number; limit?: number } = {},
    ) {
        const { startDate, endDate, categoryId, status, page = 1, limit = 50 } = options;
    ...
    async updateCostEntry(restaurantId: string, id: string, dto: UpdateCostEntryDto) {
    ...
    async deleteCostEntry(restaurantId: string, id: string, deleteRecurring = false) {
    ...
    async updateEmployee(restaurantId: string, id: string, dto: UpdateEmployeeDto) {
    ...
    async createLaborEntry(restaurantId: string, dto: CreateLaborEntryDto) {
        ...
        if (employee.benefits && typeof employee.benefits === 'object') {
            benefitsCost = Object.values(employee.benefits as any).reduce((sum: number, val: any) => sum + (Number(val) || 0), 0);
        }

        // Prorate benefits
        let payPeriodBenefits = benefitsCost / 4; // Simplification
    ...
    async findAllLaborEntries(restaurantId: string, options: any) {
        const { startDate, endDate, employeeId, isPaid, page = 1, limit = 50 } = options;
        const where: any = { restaurantId };
    ...
    async findLaborEntryById(restaurantId: string, id: string) {
        const entry = await this.prisma.laborEntry.findUnique({ where: { id }, include: { employee: true } });
        if (!entry || entry.restaurantId !== restaurantId) {
    ...
    async updateLaborEntry(restaurantId: string, id: string, dto: UpdateLaborEntryDto) {
    ...
    async calculateProfit(restaurantId: string, startDate: string, endDate: string) {
        ...
        const realFoodCost = Math.abs(Number(stockUsage._sum.totalCost || 0)) + Number(wastage._sum.totalCost || 0);
        // If real cost is 0 (not tracking), use estimated
        const foodCost = realFoodCost > 0 ? realFoodCost : (revenue * (Number(settings.targetFoodCostPercentage) / 100));
        ...
        // 4. Overhead Cost
        const months = (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24 * 30.44); // Approx months
        const monthlyFixed = Number(settings.monthlyRent) + Number(settings.monthlyUtilities) + Number(settings.monthlyInsurance) +
            Number(settings.monthlyLicenses) + Number(settings.monthlyMarketing) + Number(settings.monthlyMaintenance) +
            Number(settings.monthlySupplies) + Number(settings.monthlyOther);
