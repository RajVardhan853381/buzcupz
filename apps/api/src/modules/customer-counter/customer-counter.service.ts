...
export class CustomerCounterService {
  ...
  async recordVisit(
    ...
    if (dto.date) {
      ...
      if (dto.time) {
        ...
        visitTime = setMinutes(
          setHours(visitDate, now.getHours()),
          now.getMinutes(),
        );
      }
    } else {
    ...
    this.logger.log(
      `Recorded visit: ${dto.partySize} guests (${dto.source}) at ${format(visitTime, "HH:mm")}`,
    );
  ...
  async quickIncrement(
    ...
    return (await this.recordVisit({ partySize: count }, restaurantId))
      .liveCount;
  }
  ...
  async getDashboard(restaurantId: string) {
    const [live, today, hourlyBreakdown, peakAnalysis, trends, recentVisits] =
      await Promise.all([
  ...
  async getPeakAnalysis(restaurantId: string) {
    ...
    return {
      ...
      averagePerHour:
        activeHours > 0 ? Math.round(totalCount / activeHours) : 0,
      busiestDay: dailyTotals[0]
        ? format(new Date(dailyTotals[0].date), "EEEE, MMM d")
        : "N/A",
  ...
  async getTrends(
    restaurantId: string,
  ): Promise<{ daily: TrendData; weekly: TrendData; monthly: TrendData }> {
    const today = startOfDay(new Date());
    ...
    const [todayCount, yesterdayCount] = await Promise.all([
      ...
      this.prisma.customerVisit.aggregate({
        where: { restaurantId, visitDate: subDays(today, 1) },
        _sum: { partySize: true },
      }),
    ]);
    ...
    const [thisWeekCount, lastWeekCount] = await Promise.all([
      this.prisma.customerVisit.aggregate({
        where: { restaurantId, visitDate: { gte: thisWeekStart, lte: today } },
        _sum: { partySize: true },
      }),
      this.prisma.customerVisit.aggregate({
        where: {
          restaurantId,
          visitDate: { gte: subWeeks(thisWeekStart, 1), lt: thisWeekStart },
        },
        _sum: { partySize: true },
      }),
    ]);
    ...
    const [thisMonthCount, lastMonthCount] = await Promise.all([
      this.prisma.customerVisit.aggregate({
        where: { restaurantId, visitDate: { gte: thisMonthStart, lte: today } },
        _sum: { partySize: true },
      }),
      this.prisma.customerVisit.aggregate({
        where: {
    ...
    return {
      daily: calculateTrend(
        todayCount._sum.partySize || 0,
        yesterdayCount._sum.partySize || 0,
        "Today vs Yesterday",
      ),
      weekly: calculateTrend(
        thisWeekCount._sum.partySize || 0,
        lastWeekCount._sum.partySize || 0,
        "This Week vs Last Week",
      ),
      monthly: calculateTrend(
        thisMonthCount._sum.partySize || 0,
        lastMonthCount._sum.partySize || 0,
  ...
  async getRecentVisits(restaurantId: string, limit: number = 20) {
    ...
    return visits.map((v) => ({
      ...
      formattedDate: isToday(new Date(v.visitDate))
        ? "Today"
        : format(new Date(v.visitDate), "MMM d"),
  ...
  async updateVisit(
    ...
    if (dto.exitTime) {
      ...
      updateData.duration = differenceInMinutes(
        updateData.exitTime,
        new Date(visit.visitTime),
      );
    }

    const updated = await this.prisma.customerVisit.update({
      where: { id },
      data: updateData,
    });
    if (dto.hasOrdered)
      await this.updateDailyFootfall(restaurantId, new Date(visit.visitDate));
    return updated;
  }
  ...
  private async updateHourlyCount(
    restaurantId: string,
    date: Date,
    hour: number,
    partySize: number,
  ...
  private async updateDailyFootfall(
    restaurantId: string,
    date: Date,
  ): Promise<void> {
    const stats = await this.prisma.customerVisit.groupBy({
    ...
    const ordersPlaced = await this.prisma.customerVisit.count({
      where: { restaurantId, visitDate: date, hasOrdered: true },
    });
    ...
    const allHourly = await this.prisma.hourlyFootfall.findMany({
      where: { restaurantId, date },
    });
    allHourly.forEach((h) => {
      hourlyBreakdown[h.hour] = h.partyTotal;
    });
    ...
    await this.prisma.dailyFootfall.upsert({
      ...
      update: {
        ...
        ordersPlaced,
        totalRevenue,
        peakHour: hourlyData[0]?.hour ?? null,
        peakCount: hourlyData[0]?.partyTotal ?? null,
        hourlyBreakdown,
      },
      create: {
        restaurantId,
  ...
  private getDateRange(filters: CounterFiltersDto): {
    dateFrom: Date;
    dateTo: Date;
  } {
    const today = startOfDay(new Date());
    ...
    if (
      filters.preset === DateRangePreset.CUSTOM &&
      filters.dateFrom &&
      filters.dateTo
    ) {
      return {
        dateFrom: startOfDay(parseISO(filters.dateFrom)),
        dateTo: endOfDay(parseISO(filters.dateTo)),
      };
    }

    switch (filters.preset) {
      case DateRangePreset.TODAY:
        return { dateFrom: today, dateTo: endOfDay(today) };
      ...
      case DateRangePreset.LAST_WEEK:
        const lw = startOfWeek(subWeeks(today, 1), { weekStartsOn: 1 });
        return { dateFrom: lw, dateTo: endOfWeek(lw, { weekStartsOn: 1 }) };
      case DateRangePreset.THIS_MONTH:
        return { dateFrom: startOfMonth(today), dateTo: endOfDay(today) };
      ...
      case DateRangePreset.LAST_7_DAYS:
        return { dateFrom: subDays(today, 6), dateTo: endOfDay(today) };
      case DateRangePreset.LAST_30_DAYS:
        return { dateFrom: subDays(today, 29), dateTo: endOfDay(today) };
      default:
        return { dateFrom: today, dateTo: endOfDay(today) };
    }
  }
