...
export class ReservationProcessor extends WorkerHost {
  ...
  private async handleAutoConfirm(data: AutoConfirmJobData) {
    ...
    try {
      ...
      if (!reservation) {
        this.logger.warn(
          `Reservation ${reservationId} not found for auto-confirm`,
        );
        return;
      }
      ...
      if (reservation.status !== "PENDING") {
        this.logger.log(
          `Reservation ${reservationId} is not PENDING, skipping`,
        );
        return;
      }
      ...
      this.logger.log(
        `Auto-confirmed and notified: ${reservation.confirmationCode}`,
      );
    } catch (error) {
      this.logger.error(
        `Auto-confirm failed for ${reservationId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private async handleSendReminder(data: ReminderJobData) {
    ...
    try {
      ...
      if (!["CONFIRMED", "REMINDED"].includes(reservation.status)) {
        this.logger.log(
          `Reservation ${reservationId} status is ${reservation.status}, skipping reminder`,
        );
        return;
      }
      ...
      this.logger.error(
        `Reminder failed for ${reservationId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private async handleSendCancellation(data: CancellationJobData) {
    ...
    try {
      ...
      this.logger.log(
        `Cancellation notification sent: ${reservation.confirmationCode}`,
      );
    } catch (error) {
  ...
  private async handleTableCleanup(data: TableCleanupJobData) {
    ...
    try {
      ...
      if (table && table.status === "CLEANING") {
        ...
        this.logger.log(
          `Table ${table.number} marked as AVAILABLE after cleanup`,
        );
      }
    } catch (error) {
