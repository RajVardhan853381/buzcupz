...
export class CreateModifierDto {
    ...
    @ApiProperty({ example: 2.00, description: 'Price adjustment' })
    @Type(() => Number)
    @IsOptional()
    priceAdjustment?: number = 0;

    @ApiPropertyOptional({ enum: ['FIXED', 'PERCENTAGE', 'REPLACEMENT', 'FREE', 'QUANTITY_BASED'] })
    @IsOptional()
    @IsEnum(['FIXED', 'PERCENTAGE', 'REPLACEMENT', 'FREE', 'QUANTITY_BASED'])
    priceType?: string = 'FIXED';

    @ApiPropertyOptional({ example: 10, description: 'For percentage-based pricing' })
    @IsOptional()
    @Type(() => Number)
    ...
    @ApiPropertyOptional({ example: 0.50, description: 'Price per additional unit' })
    @IsOptional()
    @Type(() => Number)
    ...
    @ApiPropertyOptional({ example: 0.1, description: 'Amount to deduct from inventory' })
    @IsOptional()
    @Type(() => Number)
...
export class CreateModifierGroupDto {
    ...
    @ApiPropertyOptional({ type: [CreateModifierDto], description: 'Modifiers to create with the group' })
    @IsOptional()
    @IsArray()
...
export class AssignModifierGroupDto {
    ...
    @ApiPropertyOptional({ description: 'Override isRequired for this menu item' })
    @IsOptional()
    @IsBoolean()
    isRequired?: boolean;
    ...
    @ApiPropertyOptional({
        ...
        example: { 'modifier-id-1': 2.50, 'modifier-id-2': 3.00 }
    })
    @IsOptional()
    @IsObject()
