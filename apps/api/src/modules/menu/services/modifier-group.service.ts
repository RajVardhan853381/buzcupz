...
import { Prisma, ModifierGroup, Modifier, ModifierSelectionType } from '@prisma/client';
import {
...
export class ModifierGroupService {
    ...
    // CREATE
    // ============================================
    async create(dto: CreateModifierGroupDto): Promise<ModifierGroupWithModifiers> {
        ...
        if (existing) {
            throw new ConflictException(`Modifier group with name "${dto.name}" already exists`);
        }
    ...
    // READ
    // ============================================
    async findAll(query: ModifierGroupQueryDto): Promise<ModifierGroupWithModifiers[]> {
    ...
    // UPDATE
    // ============================================
    async update(id: string, dto: UpdateModifierGroupDto): Promise<ModifierGroupWithModifiers> {
        // Check exists
        await this.findOne(id);

        // Validate selection rules if changed
        if (dto.selectionType || dto.minSelections !== undefined || dto.maxSelections !== undefined) {
    ...
    // CLONE/DUPLICATE
    // ============================================
    async clone(id: string, dto: CloneModifierGroupDto): Promise<ModifierGroupWithModifiers> {
    ...
    async bulkToggleActive(ids: string[], isActive: boolean): Promise<number> {
        ...
        this.logger.log(`${isActive ? 'Activated' : 'Deactivated'} ${result.count} modifier groups`);
    ...
    private validateSelectionRules(dto: Partial<CreateModifierGroupDto>): void {
        ...
        if (selectionType === ModifierSelectionType.SINGLE && maxSelections > 1) {
            throw new BadRequestException('SINGLE selection type can only have maxSelections of 1');
        }
