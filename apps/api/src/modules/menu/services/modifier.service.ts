...
export class ModifierService {
  ...
  async create(groupId: string, dto: CreateModifierDto): Promise<Modifier> {
    ...
    const sortOrder =
      dto.sortOrder ?? (lastModifier ? lastModifier.sortOrder + 1 : 0);
  ...
  async bulkCreate(dto: BulkCreateModifiersDto): Promise<Modifier[]> {
    ...
    this.logger.log(
      `Bulk created ${modifiers.length} modifiers for group ${dto.groupId}`,
    );
  ...
  async remove(id: string): Promise<void> {
    ...
    if (orderUsage > 0) {
      ...
      this.logger.log(
        `Soft deleted modifier ${id} (used in ${orderUsage} orders)`,
      );
    } else {
      // Hard delete
  ...
  async bulkRemove(ids: string[]): Promise<number> {
    ...
    this.logger.log(
      `Bulk removed ${total} modifiers (${usedIds.length} soft, ${unusedIds.length} hard)`,
    );
  ...
  async reorder(groupId: string, dto: ReorderModifiersDto): Promise<void> {
    ...
    this.logger.log(
      `Reordered ${dto.modifierIds.length} modifiers in group ${groupId}`,
    );
  }
