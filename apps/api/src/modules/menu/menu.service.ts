...
export class MenuService {
  ...
  async assignModifierGroups(menuItemId: string, dto: AssignModifierGroupsDto) {
    ...
    await this.findItemById(
      menuItemId,
      await this.getRestaurantIdFromItem(menuItemId),
    ); // Optimized check needed normally
  ...
  async updateModifierGroupAssignment(
    menuItemId: string,
    groupId: string,
    dto: Partial<AssignModifierGroupDto>,
  ) {
    ...
    await this.prisma.$transaction(
      groupIds.map((id, idx) =>
        this.prisma.menuItemModifierGroup.update({
          where: {
            menuItemId_modifierGroupId: { menuItemId, modifierGroupId: id },
          },
          data: { sortOrder: idx },
        }),
      ),
  ...
  async copyModifiersFromItem(targetId: string, sourceId: string) {
    const source = await this.prisma.menuItemModifierGroup.findMany({
      where: { menuItemId: sourceId },
    });
    if (source.length === 0)
      throw new BadRequestException("Source has no modifiers");
