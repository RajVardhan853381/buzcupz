...
import {
  BestSellersQueryDto,
  SortByMetric,
} from "./dto/best-sellers-query.dto";
...
export class AnalyticsService {
  ...
  async getDashboard(restaurantId: string, query: AnalyticsQueryDto) {
    ...
    // Current period data
    const currentData = await this.getOrdersData(
      restaurantId,
      startDate,
      endDate,
    );
    ...
    const comparisonData = await this.getOrdersData(
      restaurantId,
      comparisonStart,
      comparisonEnd,
    );
    ...
    const revenue = {
      ...
      changePercent:
        comparisonData.totalRevenue > 0
          ? ((currentData.totalRevenue - comparisonData.totalRevenue) /
    ...
    const orders = {
      ...
      avgValue:
        currentData.orderCount > 0
          ? currentData.totalRevenue / currentData.orderCount
          : 0,
      avgItems:
        currentData.orderCount > 0
          ? currentData.itemCount / currentData.orderCount
          : 0,
      change: currentData.orderCount - comparisonData.orderCount,
      changePercent:
        comparisonData.orderCount > 0
          ? ((currentData.orderCount - comparisonData.orderCount) /
              comparisonData.orderCount) *
            100
          : 0,
    };
            ...
            ? {
                ...
                quantityPercent:
                  comparison.quantity > 0
                    ? ((quantity - comparison.quantity) / comparison.quantity) *
                      100
                    : 0,
                revenuePercent:
                  comparison.revenue > 0
                    ? ((revenue - comparison.revenue) / comparison.revenue) *
                      100
                    : 0,
              }
            : undefined,
  ...
  async getCategoryPerformance(restaurantId: string, query: AnalyticsQueryDto) {
    ...
    const comparisonMap = new Map(
      comparisonData.map((d) => [d.categoryId, d.revenue]),
    );
    ...
    return currentData.map((cat) => {
      ...
      const change =
        prevRevenue > 0 ? ((cat.revenue - prevRevenue) / prevRevenue) * 100 : 0;
