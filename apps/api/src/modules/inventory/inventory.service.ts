...
import { StockAdjustmentDto, AdjustmentType, AdjustmentReason } from './dto/stock-adjustment.dto';
import { BulkAdjustmentDto } from './dto/bulk-adjustment.dto';
import { InventoryFiltersDto, StockStatus, SortField } from './dto/inventory-filters.dto';
...
export class InventoryService {
    ...
    async createItem(dto: CreateInventoryItemDto, restaurantId: string, userId: string) {
    ...
    async findAllItems(restaurantId: string, filters: InventoryFiltersDto) {
        ...
        if (filters.storageLocation) {
            where.storageLocation = { contains: filters.storageLocation, mode: 'insensitive' };
        }
        ...
        const [items, total] = await Promise.all([
            this.prisma.inventoryItem.findMany({
                ...
                include: {
                    ...
                    _count: { select: { stockMovements: true, alerts: { where: { status: 'ACTIVE' } } } },
                },
                orderBy,
                skip,
                take: limit,
    ...
    async updateItem(id: string, dto: UpdateInventoryItemDto, restaurantId: string) {
        const existing = await this.prisma.inventoryItem.findFirst({
            where: { id, restaurantId },
        });
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const movement = await tx.stockMovement.create({
                data: {
                    ...
                    quantity: dto.type === AdjustmentType.REMOVE ? -dto.quantity : dto.quantity,
                    previousStock,
                    newStock,
                    unitCost,
                    totalCost,
    ...
    async bulkAdjust(dto: BulkAdjustmentDto, restaurantId: string, userId: string) {
        const results = [];
    ...
    async updateSupplier(id: string, dto: Partial<CreateSupplierDto>, restaurantId: string) {
        const supplier = await this.prisma.supplier.findFirst({
            where: { id, restaurantId },
        });
    ...
    async getAlerts(restaurantId: string, includeResolved = false) {
        ...
        return this.prisma.inventoryAlert.findMany({
            where,
            include: {
                inventoryItem: { select: { id: true, name: true, sku: true, currentStock: true, unit: true } },
            },
            orderBy: { createdAt: 'desc' },
        });
    }
