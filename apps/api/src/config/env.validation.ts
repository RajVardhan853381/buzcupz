...
export const envValidationSchema = Joi.object({
  ...
  API_PORT: Joi.number().integer().min(1024).max(65535).default(3001).messages({
  ...
  DATABASE_URL: Joi.string()
    ...
    .messages({
      "any.required":
        "DATABASE_URL is required. Example: postgresql://user:password@db:5432/dbname",
  ...
  JWT_SECRET: Joi.string()
    ...
    .messages({
      ...
      "string.insecure":
        'JWT_SECRET must not contain placeholder words like "change" or "example"',
    }),
  ...
  JWT_REFRESH_SECRET: Joi.string()
    ...
    .messages({
      "any.required":
        "JWT_REFRESH_SECRET is required. Generate with: openssl rand -hex 32",
      "string.min":
        "JWT_REFRESH_SECRET must be at least 32 characters for security",
      "string.insecure":
        'JWT_REFRESH_SECRET must not contain placeholder words like "change" or "example"',
    }),
